...
namespace linalg {
template <typename T>
void lobpcg_device(T* Ap, T* Bp, int n, int m, T* wp, T* vp, T* Xp = nullptr, T* Mp = nullptr,
  ...
  for (int k = 0; k < maxiter; k++) {
    ...
    tick("outer loop 3");
    /* Compute the Ritz v ector, compute batchly except in each column */
...
template <typename T>
void lobpcg_host(T* Ap, T* Bp, int n, int m, T* wp, T* vp, T* Xp = nullptr, T* Mp = nullptr,
                 double tol = 1e-8, int maxiter = 500, bool verbose = true) {
  // TODO: Bp is nullptr, use identity matrix
  // TODO: sygv for full matrix eigenvalue problem
  // TODO: if not converged, pick up the result for lowest residual
