...
TEST_F(LapackageRandom, inverse) {
  View2D<T> AxAinv("Ainv", n, n);
  KokkosBlas::gemm("N", "N", 1.0, A, Ainv, 0.0, AxAinv);

  // compare AxAinv to I
  Kokkos::parallel_for(
      "check_AxAinv", n, KOKKOS_LAMBDA(const int i) {
        for (int j = 0; j < i + 1; j++) {
          if (i == j) {
            EXPECT_NEAR(AxAinv(i, j), 1.0, tol);
          } else {
            EXPECT_NEAR(AxAinv(i, j), 0.0, tol);
          }
        }
      });
}

TEST_F(LapackageFix, inverse) {
  View2D<T> Ainv_test("Ainv_test", 10, 10);

  lapackage::inverse(A.data(), 10, Ainv_test.data());

  // compare Ainv to Ainv_test
  for (int i = 0; i < 10; i++) {
    for (int j = 0; j < i + 1; j++) {
      EXPECT_NEAR(Ainv(i, j), Ainv_test(i, j), tol);
    }
  }
}
...
TEST_F(LapackageFix, sygvx) {
  ...
  Kokkos::View<T**, Kokkos::LayoutLeft> v_test("v_test", 10, m);
...
TEST_F(LapackageRandom, sygvx) {
  View1D<T> w_test("w_test", m);
  Kokkos::View<T**, Kokkos::LayoutLeft> v_test("v_test", n, m);

  lapackage::sygvx<T>(A.data(), B.data(), n, m, w_test.data(), v_test.data());

  // // check that A * v = w * B * v
  // View1D<T> res("res", n * m);
  // for(int i = 0; i < n; i++) {
  //   for(int j = 0; j < m; j++) {
  //     res(i * m + j) = 0.0;
  //     for(int k = 0; k < n; k++) {
  //       res(i * m + j) += A(i, k) * v_test(k, j);
  //     }
  //     res(i * m + j) -= w_test(j) * B(i, j) * v_test(i, j);
  //   }
  // }
